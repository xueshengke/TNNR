function [res, Xrecover] = admm_pic(image_name, Xfull, mask, para)
...
for R = min_R : max_R
    ...
    for c = 1 : dim
        ...
        for i = 1 : max_iter
            ...
            [U, sigma, V] = svd(X);
            A = U(:, 1:R)'; B = V(:, 1:R)'; 
            [X_rec(:, :, i), iter_count] = admmAXB(A, B, X, M, known, para);
...
imshow(Xrecover ./ 255);
xlabel('image recovered by TNNR-ADMM');

if para.save_eps
    fig_eps = figure;
    imshow(Xrecover ./ 255, 'border', 'tight');
    fig_name = sprintf('TNNR-admm/result/%s_rank_%d_PSNR_%.2f_ADMM', ...
