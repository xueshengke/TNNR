function [result, X_rec] = apgl_pic(result_dir, image_name, X_full, mask, para)
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
%     main part of TNNR algorithm via APGL
% 
%     Inputs:
...
% figure;
% subplot(2,1,1);
% imshow(X_miss ./ 255);   % show the missing image
% xlabel('image with missing pixels');

figure('NumberTitle', 'off', 'Name', 'TNNR-APGL image');
subplot(1,3,1);
...
for R = min_R : max_R    % test if each rank is proper for completion
    ...
    for c = 1 : dim    % process each channel separately
        ...
        for i = 1 : max_iter
            ...
            [U, sigma, V] = svd(X);
            A = U(:, 1:R)'; B = V(:, 1:R)';
            [X, iter_count] = apglAXB(A, B, X, M, known, para);
            X_iter(:, :, c, i) = X;       
